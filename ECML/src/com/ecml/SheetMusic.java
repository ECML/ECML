 ...
 *
 *        - SheetMusic() : Create a new SheetMusic control from the given midi
 *        file and options.<br>
 * 
 *        - onDraw() : Method called to draw the SheetMuisc<br>
 *
 *        - shadeNotes() : Shade all the notes played at a given pulse time.<br>
 */
public class SheetMusic extends SurfaceView implements SurfaceHolder.Callback, ScrollAnimationListener {
	...
	public static final int LineSpace = 7; /*
											 * The space between lines in the
											 * staff
											 */
	...
	private ArrayList<Staff> staffs; /*
									 * The array of staffs to display (from top
									 * to bottom)
									 */
	...
	/**
	 * Create a new SheetMusic View. MidiFile is the parsed midi file to
	...
	public void init(MidiFile file, MidiOptions options, boolean cut, int start, int stop) {
		 ...
		 * staffs.
		 */
		ArrayList<ArrayList<MusicSymbol>> allsymbols = new ArrayList<ArrayList<MusicSymbol>>(numtracks);
		...
		for (int tracknum = 0; tracknum < numtracks; tracknum++) {
			...
			if (cut == false) {
				...
				allsymbols.add(CreateSymbols(chords, clefs, time, lastStart, cut));
			} else {
				ClefMeasures clefs = new ClefMeasures(track.getNotesBetween(start, stop), time.getMeasure());
				ArrayList<ChordSymbol> chords = CreateChords(track.getNotesBetween(start, stop), mainkey, time, clefs);
				allsymbols.add(CreateSymbols(chords, clefs, time, lastStart, cut));
		...
		/*
		 * After making chord pairs, the stem directions can change, which
		 * affects the staff height. Re-calculate the staff height.
		 */
	...
	/**
	 * Calculate the size of the sheet music width and height (without zoom
	 * scaling to fit the screen). Store the result in sheetwidth and
	 * sheetheight.
	 */
	...
	/**
	 * Adjust the zoom level so that the sheet music page (PageWidth) fits
	 * within the width. If the heightspec is 0, return the screenheight. Else,
	 * use the given view width/height.
	 */
	@Override
	...
	/**
	 * If this is the first size change, calculate the zoom level, and create
	 * the bufferCanvas. Otherwise, do nothing.
	 */
	@Override
	 ...
	 * contains the chord symbols, vertical bars, rests, and clef changes.
	 ...
	 * 
	 */
	private ArrayList<MusicSymbol> CreateSymbols(ArrayList<ChordSymbol> chords, ClefMeasures clefs, TimeSignature time, int lastStart, boolean cut) {

		ArrayList<MusicSymbol> symbols = new ArrayList<MusicSymbol>();
		symbols = AddBars(chords, time, lastStart, cut);
	...
	/**
	 * Add in the vertical bars delimiting measures. Also, add the time
	 * signature symbols.
	 * 
	 * @param chords
	 * @param time
	 ...
	 * @return
	 */
	private ArrayList<MusicSymbol> AddBars(ArrayList<ChordSymbol> chords, TimeSignature time, int lastStart, boolean cut) {
		ArrayList<MusicSymbol> symbols = new ArrayList<MusicSymbol>();
	...
	/**
	 * Return the rest symbols needed to fill the time interval between start
	 * and end. If no rests are needed, return nil.
	 * 
	 * @param time
	 *            The Time Signature
	 * @param start
	...
	private RestSymbol[] GetRests(TimeSignature time, int start, int end) {
		...
		case DottedHalf:
			...
			r2 = new RestSymbol(start + time.getQuarter() * 2, NoteDuration.Quarter);
			result = new RestSymbol[] { r1, r2 };
			return result;

		case DottedQuarter:
			r1 = new RestSymbol(start, NoteDuration.Quarter);
			r2 = new RestSymbol(start + time.getQuarter(), NoteDuration.Eighth);
			result = new RestSymbol[] { r1, r2 };
			return result;

		case DottedEighth:
			r1 = new RestSymbol(start, NoteDuration.Eighth);
			r2 = new RestSymbol(start + time.getQuarter() / 2, NoteDuration.Sixteenth);
			result = new RestSymbol[] { r1, r2 };
			return result;
	 ...
	 * left side. However, the clef can also change from measure to measure.
	 * When it does, a Clef symbol must be shown to indicate the change in clef.
	 * This function adds these Clef change symbols. This function does not add
	 * the main Clef Symbol that begins each staff. That is done in the Staff()
	 * contructor.
	 * 
	 * @param symbols
	 * @param clefs
	 ...
	 *
	 * First, each track should have a symbol for every starttime that appears
	 * in the Midi File. If a track doesn't have a symbol for a particular
	 ...
	 *
	 * The method SymbolWidths.GetExtraWidth() returns the extra width needed
	 * for a track to match the maximum symbol width for a given starttime.
	 * 
	 * @param allsymbols
	 * @param widths
	...
	private static void CreateBeamedChords(ArrayList<ArrayList<MusicSymbol>> allsymbols, TimeSignature time, int numChords, boolean startBeat) {
		...
		for (ArrayList<MusicSymbol> symbols : allsymbols) {
			...
			while (true) {
				...
				horizDistance.value = 0;
				boolean found = FindConsecutiveChords(symbols, time, startIndex, chordIndexes, horizDistance);
				if (!found) {
					break;
				}
				...
				/*
				 * What is the value of startIndex here? If we created a beam,
				 * we start after the last chord. If we failed to create a beam,
				 * we start after the first chord.
				 */
			}
		}
	}
	 ...
	 * - 4 connected chords that start on quarter note beats (4/4 or 2/4 time
	 * only)<br>
	...
	private static void CreateAllBeamedChords(ArrayList<ArrayList<MusicSymbol>> allsymbols, TimeSignature time) {
		if ((time.getNumerator() == 3 && time.getDenominator() == 4) || (time.getNumerator() == 6 && time.getDenominator() == 8)
	 ...
	 */
	private ArrayList<Staff> CreateStaffsForTrack(ArrayList<MusicSymbol> symbols, int measurelen, KeySignature key, MidiOptions options, int track,
			int totaltracks) {
		...
		while (startindex < symbols.size()) {
			...
			/*
			 * There's 3 possibilities at this point: 1. We have all the symbols
			 * in the track. The endindex stays the same.
			 * 
			 * 2. We have symbols for less than one measure. The endindex stays
			 * the same.
			 * 
			 * 3. We have symbols for 1 or more measures. Since measures cannot
			 * span multiple staffs, we must make sure endindex does not occur
			 * in the middle of a measure. We count backwards until we come to
			 * the end of a measure.
			 */

			if (endindex == symbols.size() - 1) {
				/* endindex stays the same */
			} else if (symbols.get(startindex).getStartTime() / measurelen == symbols.get(endindex).getStartTime() / measurelen) {
				/* endindex stays the same */
			} else {
	 ...
	 * Staff1 for track 0, Staff1 for track1, Staff1 for track2, Staff2 for
	 * track 0, Staff2 for track1, Staff2 for track2, ... }
	 */
	...
	/**
	 * Change the note colors for the sheet music, and redraw. This is not
	 * currently used.
	 */
	...
	/**
	 * Create a bitmap/canvas to use for double-buffered drawing. This is needed
	 * for shading the notes quickly. Instead of redrawing the entire sheet
	 * music on every shade call, we draw the sheet music to this bitmap canvas.
	 * On subsequent calls to ShadeNotes(), we only need to draw the delta (the
	 * new notes to shade/unshade) onto the bitmap, and then draw the bitmap.
	 *
	 * We include the MidiPlayer height (since we hide the MidiPlayer once the
	 * music starts playing). Also, we make the bitmap twice as large as the
	 * scroll viewable area, so that we don't need to refresh the bufferCanvas
	...
	void createBufferCanvas() {
		...
		if (scrollVert) {
			bufferBitmap = Bitmap.createBitmap(viewwidth, (viewheight + playerHeight) * 2, Bitmap.Config.ARGB_8888);
		} else {
			bufferBitmap = Bitmap.createBitmap(viewwidth * 2, (viewheight + playerHeight) * 2, Bitmap.Config.ARGB_8888);
		}
	...
	private boolean isScrollPositionInBuffer() {
		if ((scrollY < bufferY) || (scrollX < bufferX) || (scrollY > bufferY + bufferBitmap.getHeight() / 3)
	...
	/**
	 * Draw the SheetMusic to the bufferCanvas, with the given (left,top)
	 * corner.
	 * 
	 * Scale the graphics by the current zoom factor. Only draw Staffs which lie
	...
	private void drawToBuffer(int left, int top) {
		...
		Rect clip = new Rect(bufferX, bufferY, bufferX + bufferBitmap.getWidth(), bufferY + bufferBitmap.getHeight());
	...
	/**
	 * Draw the given page of the sheet music. Page numbers start from 1. A
	...
	/**
	 * Shade all the chords played at the given pulse time. First, make sure the
	 * current scroll position is in the bufferBitmap. Loop through all the
	...
	public void ShadeNotes(int currentPulseTime, int prevPulseTime, int scrollType) {
		...
		/*
		 * If the scroll position is not in the bufferCanvas, we need to redraw
		 * the sheet music into the bufferCanvas
		 */
		if (!isScrollPositionInBuffer()) {
			drawToBuffer(scrollX, scrollY);
		}
		...
		/*
		 * Loop through each staff. Each staff will shade any notes that start
		 * at currentPulseTime, and unshade notes at prevPulseTime.
		 */
		int x_shade = 0;
		int y_shade = 0;
		paint.setAntiAlias(true);
		...
		for (Staff staff : staffs) {
			bufferCanvas.translate(0, ypos);
			x_shade = staff.ShadeNotes(bufferCanvas, paint, shade1, currentPulseTime, prevPulseTime, x_shade);
			bufferCanvas.translate(0, -ypos);
			ypos += staff.getHeight();
		...
		/*
		 * We have the (x,y) position of the shaded notes. Calculate the new
		 * scroll position.
		 */
		...
		/*
		 * If the new scrollX, scrollY is not in the buffer, we have to call
		 * this method again.
		 */
		...
		/*
		 * Draw the buffer canvas to the real canvas. Translate canvas such that
		 * (scrollX,scrollY) within the bufferCanvas maps to (0,0) on the real
		 * canvas.
		 */
		SurfaceHolder holder = getHolder();
		Canvas canvas = holder.lockCanvas();
	...
	/**
	 * Scroll the sheet music so that the shaded notes are visible. If
	 * scrollGradually is true, scroll gradually (smooth scrolling) to the
	 * shaded notes. Update the scrollX/scrollY fields.
	 */
	 ...
	 * First, find the staff corresponding to the point. Then, within the staff,
	 * find the notes/symbols corresponding to the point, and return the
	 * StartTime (pulseTime) of the symbols.
	 */
